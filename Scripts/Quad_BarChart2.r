...
Plot_details <- function (avg, clusterpool, clusterpool_exp, method_exp, title, method, label){
...
Plot_4_Bar <- function (avg1, pct1, avg2, pct2, c1, c2, labels){
  ...
  Plot_1 <- Plot_details(avg1, c1, c1$avg.exp, avg1$avg.exp, "Average Expression", "avg", labels[1]) 

  Plot_2 <- Plot_details(avg2, c2, c2$avg.exp, avg2$avg.exp, "Average Expression", "avg", labels[2])
  Plot_2 <- Plot_Bween(Plot_2, Bween_pool("avg", c1, c2), "avg", c2)
  
  Plot_3 <- Plot_details(pct1, c1, c1$pct.exp, pct1$pct.exp, "%Expressed", "pct", labels[1])
  
  Plot_4 <- Plot_details(pct2, c2, c2$pct.exp, pct2$pct.exp, "%Expressed", "pct", labels[2])
